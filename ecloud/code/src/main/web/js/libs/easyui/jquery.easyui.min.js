...
(function($, win) {
    $.parser = {
        ...
        onComplete: function(_1) {
            $(window).trigger("easyui.complete", _1);
        },
...
(function($) {
    ...
    function _1f1(_1f2) {
        ...
        var str=tip.find(".tooltip-content").text();
        if(!str||!String(str).length){tip.hide()}
    };
...
(function($) {
    ...
    function _21d(_21e) {
        ...
        function _221() {
            if (opts.nobody)
            {
                _220.addClass("no-body");
                _220.children(".panel-body").addClass("no-body");
            } else
            {
                opts.nobody = false;
                _220.removeClass("no-body");
                _220.children(".panel-body").removeClass("no-body");
            }
    ...
    $.messager = {
        ...
        alert: function(_2ac, msg, icon, fn) {
            ...
            opts = $.extend({}, $.messager.defaults, {
                content: "<div class=\"" + cls + "\"></div>" + "<div class=\"js-alert\">" + opts.msg + "</div>" + "<div style=\"clear:both;\"/>",
                buttons: [{
                    text: $.messager.defaults.ok,
                    customClass:'js-alert-ok',
                    onClick: function() {
                        win.window("close");
                        opts.fn();
                    }
        ...
        confirm: function(_2ad, msg, fn) {
            ...
            var confirmIcon = opts.type == 'reminding'? 'js-confirm-reminding-ok':'js-confirm-'+opts.type;
            var cancelIcon = opts.type == 'deleting'? 'js-confirm-cancel-deleting':'js-confirm-cancel';
            opts = $.extend({}, $.messager.defaults, {
                content: "<div class=\"messager-icon messager-question\"></div>" + "<div class=\"js-confirm\">" + opts.msg + "</div>" + "<div style=\"clear:both;\"/>",
                buttons: [
                    {
                        text: $.messager.defaults.cancel,
                        customClass: cancelIcon,
                        onClick: function() {
                            win.window("close");
                            opts.fn(false);
                        }
...
(function($) {
    ...
    function _4aa(_4ab) {
        ...
        var tb = _4ac.textbox;
        if (tb.hasClass("combo"))
        {
            if (opts.disabled)
            {
                tb.addClass("disabled_combo_text");
            } else
            {
                tb.removeClass("disabled_combo_text");
            }
        }

        tb.find(".textbox-text").remove();
        var _maxlength = opts.maxlength ? ("maxlength='" + opts.maxlength + "'"): ("");
        if (opts.multiline) {
            $("<textarea " + _maxlength + " class=\"textbox-text\" autocomplete=\"off\"></textarea>").prependTo(tb);
        } else {

            $("<input " + _maxlength + " type=\"" + opts.type + "\" class=\"textbox-text\" autocomplete=\"off\">").prependTo(tb);
        }
    ...
    $.fn.textbox.defaults = $.extend({}, $.fn.validatebox.defaults, {
        ...
        inputEvents: {
            ...
            keydown: function(e) {
                ...
                setTimeout(function()
                {
                    try{t.textbox("setValue", t.textbox("getText"))}catch(err){};
                }, 10);
...
(function($) {
    ...
    function resizeChange(opts){
        if(opts.autoHeight){
            ...
            if((hei-30)>430) {
                if ($(".datagrid-wrap.panel-body")[0]) {
                    if($(".datagrid-wrap.panel-body").length>1){
                        $(".datagrid-wrap.panel-body").each(function (i,il) {
                            $(il)[0].style.height = (hei-30) + "px";
                            $(il).find(".datagrid-view")[0].style.height = (hei - 60) + "px";
                        })
                    }else{
                        $(".datagrid-wrap.panel-body")[0].style.height = (hei-30) + "px";
                        $(".datagrid-view")[0].style.height = (hei - 60) + "px";
                        //$(".datagrid-view2")[0].style.height = (hei - 60) + "px";
                    }
                }
            }else{
                if($(".datagrid-wrap.panel-body").length>1){
                    $(".datagrid-wrap.panel-body").each(function (i,il) {
                        $(il)[0].style.height = (430) + "px";
                        $(il).find(".datagrid-view")[0].style.height = (400) + "px";
                    })
                }else{
                    $(".datagrid-wrap.panel-body")[0].style.height = (430) + "px";
                    $(".datagrid-view")[0].style.height = (400) + "px";
                    //$(".datagrid-view2")[0].style.height = (400) + "px";
                }
            }
           return hei;
        }
    ...
    function _6fb(_6fc, _6fd) {
        ...
        var onLoadSuccess = $.data(_6fc, "datagrid").options.onLoadSuccess;
        if (onLoadSuccess)
        {
            onLoadSuccess();
        }
    ...
    function _6ff(_700, row) {
        ...
        var onLoadSuccess = $.data(_700, "datagrid").options.onLoadSuccess;
        if (onLoadSuccess)
        {
            onLoadSuccess();
        }
    ...
    $.fn.datagrid.methods = {
        ...
        loading: function(jq) {
            return jq.each(function() {
                ...
                opts.loadMsg=$.i18n?$.i18n.prop("easyui.datagrid.loadMsg"):opts.loadMsg;
                if (opts.loadMsg) {
    ...
    var _77a = {
        render: function(_77b, _77c, _77d) {
            ...
            //add datagrid empty tip
            var opts = $.data(_77b, "datagrid").options;
            var dom = $(this.renderTable(_77b, 0, rows, _77d));
            $(_77c).html(dom);
        ...
        renderTable: function(_783, _784, rows, _785) {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                _788.push("<tr isRow='true' id=\"" + _78c + "\" datagrid-row-index=\"" + _784 + "\" " + cls + " " + _78b + ">");
                //_788.push(this.renderRow.call(this,_783,_787,_785,_784,row));
                rowDoms.push(this.renderRow.call(this, _783, _787, _785, _784, row));
                _788.push("</tr>");
                _784++;
            }
            _788.push("</tbody></table>");
            var tipText = opts.emptyText || "\u6682\u65e0\u6570\u636e";
            var tab = $(_788.join(""));
            var tabBox = $('<div class="datagrid-tab-box"></div>');
            tabBox.append(tab);
            if (!rows.length || (rows.length == 1 && $.isEmptyObject(rows[0])))
            {
                var $emptyTip = $('<div class="datagrid-empty-tip"><div><span></span></div></div>');
                $emptyTip.find("span").text(tipText);
                tabBox.append($emptyTip);
                $emptyTip.find("span").click(function()
                {
                    if ($.isFunction(opts.onEmptyTextClick))
                    {
                        opts.onEmptyTextClick();
                    }
                });
            }
            tab.find("tr[isRow='true']").each(function(i, il)
            {
                $(this).append(rowDoms[i]);
            });
            return tabBox;
            //return _788.join("");
        },
        renderRow: function(_78d, _78e, _78f, _790, _791) {
            ...
            for (var i = 0; i < _78e.length; i++) {
                ...
                if (col) {
                    ...
                    if (col.checkbox) {
                        ...
                        if (col.formatter) {
                            cc.push('<div  useFormate="true"></div>');
                            //cc.push(col.formatter(_794,_791,_790));
                            tempRows.push(col.formatter(_794, _791, _790));
                        } else {
                            cc.push(_794);
                        }
                    }
            ...
            var dom = $(cc.join(""));
            dom.find("div[useFormate='true']").each(function(i, il)
            {
                $(this).append(tempRows[i]);
            });
            return dom;
            return cc;
            return cc.join("");
        ...
        onAfterRender: function(_7bd) {
            ...
            if (opts.finder.getRows(_7bd) && opts.finder.getRows(_7bd).length == 0)
            {
        ...
        renderEmptyRow: function(_7c0) {
            ...
            _7c2.html(this.renderTable(_7c0, 0, [{}
            ], false));
            _7c2.find("tbody *").css({
                height: 1,
        ...
        $(_920).datagrid($.extend({}, opts, {
            ...
            columns: (opts.columns && opts.columns.length) ? opts.columns: [[{
                ...
                formatter: function(_921, row, _922) {
                    var former=opts.textFormatter ? opts.textFormatter(_921, row, _922) : _921;
                    return former;
                }
            }
            ]]
...
(function($) {
    ...
    function _942(_943) {
        ...
        var _dom = $("<div class=\"combo-panel\"></div>");
        if (opts.custom)
        {
            _dom.addClass(opts.custom);
        }
        if (!_944.panel) {
            _944.panel = _dom.appendTo("body");
            _944.panel.panel({
...
(function($) {
    ...
    function _9b5(_9b6) {
        ...
        $(".right-entity")[0].onscroll=function () {
