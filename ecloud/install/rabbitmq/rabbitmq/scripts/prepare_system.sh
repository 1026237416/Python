...
function prepare_rabbitmq_system ()
{
    check_system
	check_hostname
	setting_yum_source
...
function check_hostname ()
{
	...
	then
		...
		read -p "please input hostname for system [Default:RabbitMQ] :" setting_hostname
...
function setting_yum_source ()
{
	...
	echo "[rabbitmq]" > /etc/yum.repos.d/rabbitmq.repo
	echo "name=rabbitmq" >> /etc/yum.repos.d/rabbitmq.repo
	echo "baseurl=file:///mnt/rabbitmq" >> /etc/yum.repos.d/rabbitmq.repo
	echo "gpgcheck=0" >> /etc/yum.repos.d/rabbitmq.repo
	echo "enabled=1" >> /etc/yum.repos.d/rabbitmq.repo
	echo "proxy=_none_" >> /etc/yum.repos.d/rabbitmq.repo
	
	tar xzvfm ${INSTALL_PATH}/packages/rabbitmq.tar.gz -C /mnt
}
...
{
	echo "================================================================================================" | tee -a ${LOG_FILE}
	echo "                                  Update rabbitmq node                                           " | tee -a ${LOG_FILE}
	echo "================================================================================================" | tee -a ${LOG_FILE}
