...
function check_datadase_hostname ()
{
	...
	then
		...
		read -p "please input hostname for system [Default:Redis] :" setting_hostname
...
{	
	...
	echo "[redis]" > /etc/yum.repos.d/redis.repo
	echo "name=redis" >> /etc/yum.repos.d/redis.repo
	echo "baseurl=file:///mnt/redis" >> /etc/yum.repos.d/redis.repo
	echo "gpgcheck=0" >> /etc/yum.repos.d/redis.repo
	echo "enabled=1" >> /etc/yum.repos.d/redis.repo
	echo "proxy=_none_" >> /etc/yum.repos.d/redis.repo
	
	tar xzvfm ${INSTALL_PATH}/packages/redis.tar.gz -C /mnt
}
...
{
	echo "================================================================================================" | tee -a ${LOG_FILE}
	echo "                                  Update redis node                                           " | tee -a ${LOG_FILE}
	echo "================================================================================================" | tee -a ${LOG_FILE}
