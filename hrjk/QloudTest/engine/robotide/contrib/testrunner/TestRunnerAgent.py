...
class TestRunnerAgent:#测试案例运行代理
...
class RobotDebugger(object):#robot调试器
...
class RobotKillerServer(SocketServer.TCPServer):#robot服务关闭
    allow_reuse_address = True
    def __init__(self, debugger):
        SocketServer.TCPServer.__init__(self, ("",0), RobotKillerHandler)
        self.debugger = debugger

class RobotKillerHandler(SocketServer.StreamRequestHandler):#robot处理器关闭
    def handle(self):
