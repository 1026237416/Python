...
class PybotProfile(BaseProfile):#pybot型
    ...
    def ArgumentsPanel(self, parent):
        '''
        panel = wx.Panel(parent, wx.ID_ANY)
        label = Label(panel, label=u"参数: ")#"Arguments: "
    ...
    def TagsPanel(self, parent):
        ...
        include_cb = self._create_checkbox(panel, self.apply_include_tags,
                                           u"仅使用下列标签运行测试案例")#"Only run tests with these tags"
        exclude_cb = self._create_checkbox(panel, self.apply_exclude_tags,
                                           u"跳过下列标签运行测试案例")#"Skip tests with these tags"
...
class CustomScriptProfile(PybotProfile):#自定义脚本型
