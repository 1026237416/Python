...
ERROR_ILLEGAL_CHARACTERS = "Filename contains illegal characters"#文件名非法描述错误
ERROR_EMPTY_FILENAME = "Empty filename"#空文件名错误
ERROR_NEWLINES_IN_THE_FILENAME = "Newlines in the filename"#文件名中有换行符错误
ERROR_FILE_ALREADY_EXISTS = "File %s already exists"#文件已存在错误
'''基于文件名验证文件是否存在'''
class BaseNameValidator(object):#基本名称验证器

    def __init__(self, new_basename):
        self._new_basename = new_basename

    def validate(self, context):#验证
    ...
    def _file_exists(self, filename):#判断文件是否存在
