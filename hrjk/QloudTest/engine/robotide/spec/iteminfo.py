...
class ItemInfo(object):#项目信息
    ...
    def longname_begins_with(self, prefix):#名字以什么开始
        return utils.normalize(self.longname).startswith(prefix)

    def is_library_keyword(self):#是否是库关键词
        return False

    def is_user_keyword(self):#是否是用户关键词
        return not self.is_library_keyword()

    def __cmp__(self, other):#对比
...
class VariableInfo(ItemInfo):#变量信息
    ...
    def _source_name(self, source):#资源名字
...
class ArgumentInfo(VariableInfo):#参数信息
...
class LocalVariableInfo(VariableInfo):#本地变量信息
...
class _KeywordInfo(ItemInfo):#关键词信息
...
class _XMLKeywordContent(_KeywordInfo):#xml文件关键词内容
    ...
    def _parse_args(self, node):#解析参数
...
class _UserKeywordInfo(_KeywordInfo):#用户关键词信息
    ...
    def _is_scalar(self, arg):#是否是标量
        return arg.startswith('$')

    def _parse_name_and_default(self, arg):#分析名字和默认值
        parts = arg.split('=', 1)
    ...
    def _strip_var_syntax_chars(self, string):#char型变量语法分离
        return string[2:-1]

    def _is_list(self, arg):#是否是list变量
        return arg.startswith('@')

    def _parse_vararg(self, arg):#分析参数变量
...
class ResourceUserKeywordInfo(_UserKeywordInfo):#资源用户关键词信息
    _type = 'resource file'
