...
class LibraryManager(Thread):#库管理器
    ...
    def _initiate_database_connection(self):#数据库连接初始化
        self._database = LibraryDatabase(self._database_name)

    def get_new_connection_to_library_database(self):#获取与库数据库的新连接
    ...
    def _handle_fetch_keywords_message(self, message):#处理提取关键字消息
    ...
    def _fetch_keywords(self, library_name, library_args):#获取关键字
        try:
            path = get_path(
    ...
    def _handle_insert_keywords_message(self, message):#处理插入关键字消息
    ...
    def _update_database_and_call_callback_if_needed(self, library_key, keywords, callback):#更新数据库并调用回调（如果需要）
    ...
    def fetch_keywords(self, library_name, library_args, callback):#获取关键字
        self._messages.put(('fetch', library_name, library_args, callback))

    def get_and_insert_keywords(self, library_name, library_args):#获得并插入关键字
