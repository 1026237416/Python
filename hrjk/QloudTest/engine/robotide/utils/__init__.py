...
def html_format(text):#html格式
    return robot.utils.html_format(text)


def name_from_class(item, drop=None):#从类中获取的名字
...
def split_value(value, sep='|'):#分离值
    if not value:
        return []
    return [v.strip() for v in _split_value(value, sep)]


def _split_value(value, sep):#分离值（属于隐私方法，通常情况下外部不会调用，但不意味着不能调用）
    if '\\' not in value:
...
def join_value(value, sep='|', joiner=None):#组合值
    if not joiner:
        joiner = ' %s ' % sep
    return joiner.join([v.replace(sep, '\\' + sep) for v in value])


def find_from_pythonpath(name):#找到python路径
...
def replace_extension(path, new_extension):#代替延伸
    base = path.rsplit('.', 1)
    return '%s.%s' % (base[0], new_extension.lower())


def overrides(interface_class):#覆盖
    """
    ...
    """
    def overrider(method):#覆盖者
        assert(method.__name__ in dir(interface_class))
        return method
    return overrider


def is_same_drive(path1, path2):#是否是相同的驱动
