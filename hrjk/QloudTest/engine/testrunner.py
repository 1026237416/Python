...
class TestRunner(object):  # 测试案例的运行
    ...
    def _result_handler(self, event, *args):
        ...
        if event == 'start_test':
            ...
            testname = args[0]
            print 'longname:{},testname:{}'.format(longname, testname)
    ...
    def get_command(self, pythonpath, monitor_width, names_to_run, dir_path):
        ...
        self._write_argfile(argfile, self._create_standard_args(
            command, pythonpath, monitor_width, names_to_run))
        return command
    ...
    def _create_standard_args(
            self, command, pythonpath, monitor_width, names_to_run):
        standard_args = []
        ...
        for suite in names_to_run:
            standard_args += ['--suite', suite]
...
class StreamReaderThread(object):  # 线程流的读取
    ...
    def pop(self):
        '''Changed by lill
        2017年12月20日，将“.encode('utf-8')”注释，解决UnicodeDecodeError
        Changed by lill
        2017年12月21日，增加了编码格式的判断，解决了显示乱码的问题
        '''
        result = ""
        for _ in xrange(self._queue.qsize()):
            try:
                ...
                pass
        '''
        fencoding = chardet.detect(result)['encoding']
        if fencoding != None:
            pass
        else:
            fencoding = 'GBK'
        ...
        '''
        return result  # .decode(fencoding).encode('utf-8')
...
# thread-safe manner.
class RideListenerServer(SocketServer.TCPServer):  # ride服务监听
