...
class PluginLoader(object):#插件加载器
    ...
    def enable_plugins(self):#启用插件
        for p in self.plugins:
            p.enable_on_startup()

    def _find_classes(self, load_dirs):#发现类
        classes = []
    ...
    def _is_plugin_class(self, path, cls):#判断是否是插件类
        try:
    ...
    def _find_python_files(self, load_dirs):#发现python文件
        files = []
    ...
    def _import_classes(self, path):#导入类
