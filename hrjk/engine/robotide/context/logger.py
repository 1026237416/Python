...
class Logger(object):#日志文件
    ...
    def report_parsing_errors(self):#报告错误分析
    ...
    def _format_parsing_error_line(self, line):#排格式错误分析
        if ':' not in line:
            return line
        index = line.index(':') + 1
        return line[:index] + '\n\t' + line[index:]
    ...
    def _is_logged(self, level):#判断是否是错误或警告
        return level.upper() in ['ERROR', 'WARN']

    def _is_ignored_warning(self, msg):#判断警告是否忽略
...
'''
class ParsingErrorDialog(Dialog):#错误分析窗口
