...
class LibraryCache(object):#库的存储
    ...
    def get_library_keywords(self, name, args=None, alias=None):#获取库关键词
    ...
    def _alias_to_args(self, alias, args):#参数别名
        if alias:
            if args:
    ...
    def get_default_keywords(self):#获取默认关键词
        return self._default_kws[:]

    def _build_default_kws(self):#构建默认关键词
        kws = []
    ...
    def _get_name_and_args(self, libsetting):#获取名字和参数
...
class ExpiringCache(object):#内存终止
    ...
    def _is_valid(self, key_time):#判断是否是有效的
