...
class Publisher(object):#公示
    ...
    def subscribe(self, listener, topic, key=None):#赞成
    ...
    def unsubscribe(self, listener, topic, key=None):#不赞成
    ...
    def unsubscribe_all(self, key=None):#不赞成所有
...
class _ListenerWrapper:#监听封装
    ...
    def _get_topic(self, topic):#获取主题
        if not isinstance(topic, basestring):
            topic = topic.topic
        return topic.lower()

    def wraps(self, listener, topic):#判断是否封装
