...
class _AbstractValidator():#抽象验证器
    """Implements methods to keep wxPython happy and some helper methods."""

    def Clone(self):
        return self.__class__()

    def TransferFromWindow(self):#从窗口转移
        return True

    def TransferToWindow(self):#转移到窗口
        return True

    def Validate(self, win):#验证
    ...
    def _show_error(self, message, title='Validation Error'):#显示错误
        """
    ...
    def _set_focus_to_text_control(self, ctrl):#将焦点放在文本控制上
        ctrl.SetFocus()
        ctrl.SelectAll()


class TimeoutValidator(_AbstractValidator):#超时验证器
...
class ArgumentsValidator(_AbstractValidator):#参数验证器
    ...
    def _validate_list_args_in_correct_place(self, types):#验证列表参数在正确的位置
        if 3 in types and types.index(3) != len(types) - 1:
            return "List variable allowed only as the last argument"
        return None

    def _validate_req_args_in_correct_place(self, types):#验证必需参数在正确的位置
        prev = 0
        for t in types:
            if t < prev:
...
class NonEmptyValidator(_AbstractValidator):#非空验证器
...
class SuiteFileNameValidator(NonEmptyValidator):#套件文件名验证器
...
class DirectoryExistsValidator(_AbstractValidator):#目录是否存在验证器
...
class NewSuitePathValidator(_AbstractValidator):#新套件路径验证器
...
class _NameValidator(_AbstractValidator):#名字验证器
...
class TestCaseNameValidator(_NameValidator):#测试案例名验证器
    @property
    def _validation_method(self):
        return self._controller.validate_test_name


class UserKeywordNameValidator(_NameValidator):#用户关键字名验证器
    @property
    def _validation_method(self):
        return self._controller.validate_keyword_name


class ScalarVariableNameValidator(_NameValidator):#标量变量名验证器
    @property
    def _validation_method(self):
        return self._controller.validate_scalar_variable_name


class ListVariableNameValidator(_NameValidator):#列表变量名验证器
    @property
    def _validation_method(self):#验证方法
